...
class Membership_Promo_Module extends Membership_Base_Module
{
	...
	public function enqueueTutorialAssets() {
		...
		/*wp_register_script(
			'MembershipApi',
			$this->getModule('base')->getAssetsPath() . '/js/api.js',
			array('jquery'),
			$this->getModule('base')->getConfig('plugin_version')
		);
		...
		$assetsModule->enqueueScripts($scripts, MBS_BACKEND, true);
	}
	...
	public function pointers()
	{
		return array(
			...
			array(
				...
				'target' => '.sc-tabs [data-target="groups"]',
				'edge'	  => 'top',
				'align'	 => 'left',
				'nextURL' => $this->generateUrl('membership', 'index') . '#groups',
			),
			array(
				'id' => 'step-6',
			...
			array(
				...
				'target' => '.sc-tabs [data-target="reports"]',
				'edge'	  => 'top',
				'align'	 => 'left',
				'nextURL' => $this->generateUrl('membership', 'index') . '#reports',
			),
			array(
				'id' => 'step-9',
